...
int main() {
  ...
  if (task_manager_execute_task_with_certain_priority_or_higher(head, 0) == PRIORITY_IS_NOT_FOUND) printf("no more 0 or higher\n");

  task_manager_print(head);

  task_manager_execute_task_with_certain_priority_or_higher(head, 8);
  if (task_manager_execute_task_with_certain_priority_or_higher(head, 8) == PRIORITY_IS_NOT_FOUND) printf("no more 8 or higher\n");
