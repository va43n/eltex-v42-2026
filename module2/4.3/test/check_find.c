...
START_TEST(check_find_1) {
  ...
  printf(
      "%s real other\n",
      phone_book_get_page(head, phone_book_find_page_by_full_name(head, "ich"))
          ->other);
  ck_assert_int_eq(
      strcmp(phone_book_get_page(head,
  ...
  ck_assert_int_eq(
      strcmp(phone_book_get_page(head,
             ...
             OTHER1),
      0);
  printf("another printf %s\n",
         phone_book_get_page(head, phone_book_find_page_by_number(head, "-45"))
             ->other);
  ck_assert_int_eq(strcmp(phone_book_get_page(
