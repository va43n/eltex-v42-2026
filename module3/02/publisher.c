...
int do_publisher_activity() {
  ...
  if (!check_if_queue_is_created(key)) {
    fprintf(stderr,
            "ERROR: DO_PUBLISHER_ACTIVITY - the queue does not exist.\n");
    return FAILURE;
  }
  int msgid = msgget(key, 0666 | IPC_CREAT);
  if (msgid == -1) {
    fprintf(stderr,
            "ERROR: DO_PUBLISHER_ACTIVITY - some error occured while working "
            "with queue.\n");
    perror("msgget");
    return FAILURE;
  }
  ...
  while (!is_signal) {
    ...
    if (ret == -1) {
      ...
      fprintf(stderr,
              "ERROR: DO_PUBLISHER_ACTIVITY - some error occured while trying "
