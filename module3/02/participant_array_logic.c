...
int participant_push_back(participant** ps, size_t* size, int pid, char* topic, char* payload) {
  ...
  *ps = tmp;
  ...
  (*ps)[*size - 1].topics[0] = (char*)malloc(sizeof(char) * (strlen(topic) + 1));
  strcpy((*ps)[*size - 1].topics[0], topic);
  (*ps)[*size - 1].payloads[0] = (char*)malloc(sizeof(char) * (strlen(payload) + 1));
...
int participant_add_message_by_index(participant** ps, size_t size, size_t index, char* topic, char* payload) {
  (*ps)[index].size++;
  char** tmp = (char**)realloc((*ps)[index].topics, sizeof(char*) * (*ps)[index].size);
  (*ps)[index].topics = tmp;

  (*ps)[index].topics[(*ps)[index].size - 1] = (char*)malloc(sizeof(char) * (strlen(topic) + 1));
  strcpy((*ps)[index].topics[(*ps)[index].size - 1], topic);

  tmp = (char**)realloc((*ps)[index].payloads, sizeof(char*) * (*ps)[index].size);
  (*ps)[index].payloads = tmp;

  (*ps)[index].payloads[(*ps)[index].size - 1] = (char*)malloc(sizeof(char) * (strlen(payload) + 1));
